...
export default function EditArticlePage({ params }: EditArticlePageProps) {
  ...
  const loadArticle = async () => {
    try {
      ...
      if (isNaN(articleId)) {
        toast({
          title: "خطا",
          description: "شناسه مقاله نامعتبر است",
          variant: "destructive",
        })
      ...
      if (result.error || !result.data) {
        toast({
          title: "خطا",
          description: result.error || "مقاله یافت نشد",
          variant: "destructive",
        })
        router.push("/admin/articles")
        return
      ...
      toast({
        title: "خطا",
        description: "خطا در بارگیری مقاله",
        variant: "destructive",
      })
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const articleId = Number.parseInt(params.id)
      const result = await updateArticle(articleId, submitData)
  ...
  if (isPageLoading) {
    return (
      <div className="flex justify-center items-center min-h-[400px]">
        ...
        <span className="mr-2">در حال بارگذاری مقاله...</span>
      </div>
    )
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">ویرایش مقاله موجود</p>
          </div>
        </div>
